					...
					if (list.size() == 0) {
						...
						for (BookInfoDTO dto : list) {
							...
							System.out.println(String.format("| %-3s| %-20s\t\t| %-10s\t| %-6s\t| %-6s|", dto.getBook_code(), truncateString(dto.getBookName(), 20),
									truncateString(dto.getAuthor_name(), 10), truncateString(dto.getPublisher_name(), 6) , (dto.getBook_condition().equals("대출가능") ? "대출가능":"대출불가" )));						
						}					
					}
					System.out.println(LINE);
  ...
  // * 대출 중 도서 예약
	...
	public void loan() {
		...
		try {
			...
			System.out.println();
			System.out.println(LINE);
		    System.out.println(String.format("| %-4s|%-4s\t|\t\t%-11s\t| %-11s\t| %-10s|", "대출번호", "도서코드", "책 제목","대출일자", "반납예정일자"));
			for (LoanDTO dto : list) {
				System.out.println("----------------------------------------------------------------------------");
				System.out.println(String.format("| %-5s| %-6s | %-10s\t | %-10s\t| %-12s|", dto.getLoan_code(), dto.getBook_code(), truncateString(dto.getBookname(), 20),
