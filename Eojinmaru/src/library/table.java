...
-- loan_reservation 테이블 제약조건
ALTER TABLE loan_reservation add constraint fk_loan_reservation_userinfo foreign key (user_code) REFERENCES user_info (user_code) ;
ALTER TABLE loan_reservation add constraint fk_loan_reservation_bookcode foreign key (book_code) REFERENCES book (book_code) ;
...
ALTER TABLE Loan add constraint fk_loan_user_info foreign key (user_code) REFERENCES user_info (user_code);
...
INSERT INTO user_info (user_code, user_id, user_pwd, user_name, user_birth, user_tel, user_email, user_address, loan_renewaldate) 
